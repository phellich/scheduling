...
def initialize_activities(): # utiliser les data et enlever le none by default
...
def initialize_activities_2(df): # utiliser les data et enlever le none by default

    num_activities = len(df) + 2 # +2 for dawn and dusk
    activities_array = (Activity * num_activities)()
...
def initialize_start_utility(): 
    num_activities = 60  # This value should be the same as you've used before
...
def initialize_duration_utility(): 
    num_activities = 60  # This value should be the same as you've used before
...
def main_test():
    ...
    activities_array = initialize_activities()
    pyduration_Ut = initialize_duration_utility()
    pytime_Ut = initialize_start_utility()
...
def main_iterative():
    ...
    num_activities = len(activity_csv) + 2
    activities_array = initialize_activities_2(activity_csv) 
    ...
    for index, individual in tqdm(population_csv.iterrows(), total=population_csv.shape[0]):
        ...
        schedules.append(schedule_data) # not ok like that : how to keep the sequence in the csv

    results = pd.DataFrame({
        'execution_times': execution_times,
        'DSSR_iterations': DSSR_iterations,
        'schedules': schedules  # Assuming that you can represent a schedule as a single string or value for now.
    })
    results.to_csv('./data_output/schedules.csv', index=False)

    # read the data activities and individuals
    # faire une boucle for pour initialiser activities
    # initialiser temp, iter, final schedule, id
    # iterate over individuals
        # finaliser activties avce dusk and dawn 
        # call lib.set_activities_pointer()
        # call main
        # append toutes les infos necessaires a leur vecteur
    # end for
    # tout ecrire dans un csv
...
if __name__ == "__main__":
 ...
 # 3 fichiers
 # puis bousiller la vision de utilite
 # comment capturer les preferences de chacun
